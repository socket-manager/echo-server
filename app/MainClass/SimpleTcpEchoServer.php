...
use SocketManager\Library\ISimpleSocketTcpServer;
 ...
 * シンプルソケットの初期化と実行
 */
class SimpleTcpEchoServer extends Console
{
    /**
     * @var string $identifer アプリケーション識別子
     */
    protected string $identifer = 'app:simple-tcp-echo-server {port?}';

    /**
     * @var string $description コマンド説明
     */
    protected string $description = 'シンプルソケット TCP エコーサーバー';
    ...
    public function exec()
    {
         ...
         **********************************************************************/
        $generator = new SimpleSocketGenerator(SimpleSocketTypeEnum::TCP_SERVER, $this->host, $this->port);
        ...
        (
            ...
            function(string $p_level, array $p_param): void
            {
                ...
                error_log($log, 3, "./logs/socket-manager/{$filename}_SIMPLE_TCP_S.log");
            }
        );
        ...
        (
            ...
            {
                $cid = '';
                $dat = $p_simple_socket->recv($cid);
                if($dat === null)
                {
                    return;
                }
                $p_simple_socket->send($cid, $dat);
            }
        );
