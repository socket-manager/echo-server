...
use SocketManager\Library\ISimpleSocketTcpClient;
 ...
 * シンプルソケットの初期化と実行
 */
class SimpleTcpEchoClient extends Console
{
    /**
     * @var string $identifer アプリケーション識別子
     */
    protected string $identifer = 'app:simple-tcp-echo-client {port?}';

    /**
     * @var string $description コマンド説明
     */
    protected string $description = 'シンプルソケット TCP エコークライアント';
    ...
    public function exec()
    {
         ...
         **********************************************************************/
        $generator = new SimpleSocketGenerator(SimpleSocketTypeEnum::TCP_CLIENT, $this->host, $this->port);
        ...
        (
            ...
            function(string $p_level, array $p_param): void
            {
                ...
                error_log($log, 3, "./logs/socket-manager/{$filename}_SIMPLE_TCP_C.log");
            }
        );
        ...
        (
            ...
            {
                $dat = $p_simple_socket->recv();
                if($dat === null)
                {
                    return;
                }
                $p_simple_socket->logWriter('info', ['echo back' => "data[{$dat}]"]);
            }
        );
