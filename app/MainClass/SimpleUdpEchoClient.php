 ...
 * シンプルソケットの初期化と実行
 */
class SimpleUdpEchoClient extends Console
{
    /**
     * @var string $identifer アプリケーション識別子
     */
    protected string $identifer = 'app:simple-udp-echo-client {port?}';

    /**
     * @var string $description コマンド説明
     */
    protected string $description = 'シンプルソケット UDP エコークライアント';
    ...
    public function exec()
    {
         ...
         **********************************************************************/
        $generator = new SimpleSocketGenerator(SimpleSocketTypeEnum::UDP, null, null);
        ...
        (
            ...
            function(string $p_level, array $p_param): void
            {
                ...
                error_log($log, 3, "./logs/socket-manager/{$filename}_SIMPLE_UDP_C.log");
            }
        );
        ...
        $ret = $ins->sendto($this->host, $this->port, $dat);
